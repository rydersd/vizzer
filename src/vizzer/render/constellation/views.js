...
const velRate=value=>Number(value||0).toFixed(2),velLead=(days,samples)=>days==null?`— (n=${samples||0})`:`${Number(days).toFixed(1)} d (n=${samples})`,velTokens=value=>value==null?'—':String(value),velCount=(n,noun)=>`${n} ${noun}${n===1?'':'s'}`;
const velocityStat=(value,unit,detail='')=>`<div class="velocitystat"><strong>${esc(value)}</strong><span>${esc(unit)}</span>${detail?`<small>${esc(detail)}</small>`:''}</div>`;
const VELOCITY_EMPTY='No persisted lifecycle, merge, build, or spend evidence yet.';
function velocityStats(v){const w=v.window,r=v.recent,span=`${w.days} days`,stats=[velocityStat(velRate(w.shippedPerDay),'shipped / day',`${w.shipped} shipped in ${span}`),velocityStat(velRate(w.prsPerDay),'PRs merged / day',`${velCount(w.prs,'PR')} in ${span}`),velocityStat(velLead(w.leadMedianDays,w.leadSamples),'median lead time, in-flight → shipped','items that jumped straight to shipped are not counted'),velocityStat(velRate(w.buildsPerDay),'owner builds / day',`${velCount(w.builds,'build')} in ${span}`),velocityStat(String(v.wip.length),'in flight now','currently in-flight/building; not evidence of completion')].join(''),recent=r.days?`<p class="viewsubmeta">Last ${r.days} days: ${velRate(r.shippedPerDay)} shipped/day · ${velRate(r.prsPerDay)} PRs/day · lead ${velLead(r.leadMedianDays,r.leadSamples)} · ${velRate(r.buildsPerDay)} builds/day</p>`:'';return {span,stats,recent};}
function renderVelocityStrip(){const v=DATA.velocity,head='<section class="viewsection velocity velocitystrip"><h2>Velocity',link='<p class="viewsubmeta"><a class="velocitylink" href="#velocity">Open Velocity →</a></p>';if(!v||!v.window||!v.window.days)return `${head}</h2><p class="viewsubmeta">${VELOCITY_EMPTY}</p>${link}</section>`;const {span,stats,recent}=velocityStats(v);return `${head} <span>${esc(span)}</span></h2><div class="velocitystats">${stats}</div>${recent}${link}</section>`;}
function renderVelocityView(){const v=DATA.velocity;if(!v||!v.window||!v.window.days)return panelHead('Velocity','Flow metrics over persisted evidence only: lifecycle flips, merged PRs, owner builds, declared spend.')+`<p class="viewsubmeta">${VELOCITY_EMPTY}</p>`;const {span,stats,recent}=velocityStats(v),dayRows=v.days.map(day=>`<tr><td>${esc(day.day)}</td><td>${esc(day.host||'—')}</td><td class="num">${day.shipped}</td><td class="num">${day.prs} (${day.prsVisible}/${day.prsInfra})</td><td class="num">${day.builds}</td><td class="num">${day.started}</td><td class="num">${day.bugOpened}/${day.bugClosed}</td><td class="num">${esc(velTokens(day.tokensM))}</td></tr>`).join(''),hostRows=(v.hosts||[]).map(host=>`<tr><td>${esc(host.host)}</td><td class="num">${host.days}</td><td class="num">${host.shipped}</td><td class="num">${velRate(host.shippedPerDay)}</td><td class="num">${host.prs}</td><td class="num">${velRate(host.prsPerDay)}</td><td class="num">${esc(velLead(host.leadMedianDays,host.leadSamples))}</td><td class="num">${esc(velTokens(host.tokensM))}</td></tr>`).join(''),days=`<details class="velocitydetails" open><summary>Per day, newest first · ${v.days.length} days</summary><table class="viewtable velocitytable"><thead><tr><th>Day</th><th>Host</th><th class="num">Shipped</th><th class="num">PRs</th><th class="num">Owner builds</th><th class="num">→ In flight</th><th class="num">Bug-gaps opened/closed</th><th class="num">Tokens (M)</th></tr></thead><tbody>${dayRows}</tbody></table><p class="velocitycaveat">Tokens show — when no spend was declared for that day; zero is not inferred.</p></details>`,hosts=`<h3>By host — baseline</h3><table class="viewtable velocitytable"><thead><tr><th>Host</th><th class="num">Days</th><th class="num">Shipped</th><th class="num">Shipped / day</th><th class="num">PRs</th><th class="num">PRs / day</th><th class="num">Lead median</th><th class="num">Tokens (M)</th></tr></thead><tbody>${hostRows}</tbody></table>`,warnings=(v.warnings||[]).length?`<p class="velocitycaveat">${v.warnings.map(esc).join('<br>')}</p>`:'',meta=`Window: last ${v.window.days} days ending ${v.anchor||'unrecorded'} · host ${v.currentHost||'unrecorded'} · merges read from ${v.mergeRef||'no git'}`;return panelHead('Velocity','Flow metrics over persisted evidence only: lifecycle flips, merged PRs, owner builds, declared spend.')+`<p class="viewsubmeta">${esc(meta)}</p><div class="velocitystats">${stats}</div>${recent}${days}${hosts}${warnings}`;}
function renderDashboard(entries){
  ...
  return panelHead('Dashboard','Project documents, recommended next actions, and tracked work.')+renderVelocityStrip()+radar+projectHome+
...
function renderCurrentView(){
  ...
  viewPanel.innerHTML=currentView==='dashboard'?renderDashboard(entries)
    :currentView==='velocity'?renderVelocityView()
