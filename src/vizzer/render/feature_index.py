...
from .common import item_link, source_link_prefix, status_cell
...
def _table(items: list[Item], cfg: Config, prefix: str) -> list[str]:
    ...
    for item in sorted(items, key=lambda candidate: candidate.id):
        ...
        lines.append(
            ...
            f"| {item.release or '—'} | {item_link(item, prefix)} |"
        )
    return lines
...
def render(graph: Graph, cfg: Config, root: Path) -> dict[str, str]:
    ...
    for top in tops:
        ...
        if direct:
            lines.extend(["### (root)", "", *_table(direct, cfg, prefix), ""])
        ...
        for child in children:
            ...
            lines.extend(
                [f"### {child.title}", "", *_table(child_items, cfg, prefix), ""]
            )
