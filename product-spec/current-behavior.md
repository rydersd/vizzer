...
| Owner question navigation | With two or more open owner questions, the served dossier pins Previous \| "N of M" \| Next above the title; it steps across stories in graph order, keeps drafts, does not wrap, hides for one question and in file://, and moves on after answers are provided. | [Navigator](../src/vizzer/render/constellation/question_navigation.js) | [Tests](../tests/test_question_navigation.py), [Story](stories/owner-question-prev-next-navigation.md) |
...
| Adversarial test review | Complex, end-to-end, performance, mutation and other high-impact tests use bounded design packets, independent verdicts and durable provider-routed requests. Constellation dossiers expose risk, execution state, outcomes, challenges and improvement opportunities. | [Queue](../src/vizzer/discussion_queue.py), [constellation](../src/vizzer/render/constellation.py) | [Packet tests](../tests/test_test_review_packets.py), [queue tests](../tests/test_discussion_queue.py), [guide](../wiki/guides/adversarial-test-design.md) |
