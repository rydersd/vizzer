...
def test_reply_is_bound_to_owner_and_rejects_duplicates(root):
    ...
    reply=append_message(root,'foundation','r-1','Define the outcome first.',author='Codex',reply_to='q-1')
    assert reply['replyToHash']==owner['sha256']
    assert read_area(root,'foundation')['pending']==0
    with pytest.raises(ValueError,match='already answered'):
        append_message(root,'foundation','r-2','Another answer',author='Claude',reply_to='q-1')
    with pytest.raises(ValueError,match='owner message'):
        append_message(root,'foundation','r-3','Wrong parent',author='Codex',reply_to='missing')
