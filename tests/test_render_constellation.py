...
const ctx=new Proxy({measureText:text=>({width:String(text).length*7}),createLinearGradient:()=>({addColorStop(){}})},{get:(o,k)=>o[k]??(()=>{}),set:(o,k,v)=>(o[k]=v,true)}),windowListeners={};
...
def test_constellation_uses_lightweight_outline_glyphs_and_strong_activity_pulse(tmp_path):
    ...
    assert "if(n.g==='shipped')trianglePath(p.x,p.y,rr)" in html
    assert "else if(n.g==='buggap')xPath(p.x,p.y,rr*.7)" in html
    assert "drawsHollowCircle(n)?Math.max(.35,versionOpacity(n)):1" in html
...
def test_constellation_question_cards_are_selectable_but_static_files_are_read_only(tmp_path):
    ...
    assert "reconcileAcceptedDecisions(accepted.decisions,accepted.revision,{showFromTop:true,notes:accepted.notes})" in html
    ...
    assert "await recordQuestionAnswers(forms,answers," in queue_logic
    assert "reconcileAcceptedDecisions(accepted.decisions,accepted.revision,{showFromTop:true,notes:accepted.notes})" in queue_logic
...
def test_constellation_separates_progress_version_opacity_and_has_proximity_hit_states(tmp_path):
    ...
    assert "% fill progress" in html and "versionChannelName(n)" in html
    assert "'circle stroke width':'ring'" in html
